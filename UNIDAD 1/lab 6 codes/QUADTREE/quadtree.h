...
public:
    ...
    explicit QuadLeaf(T value) : val(value) {}
...
public:
    ...
    ~QuadNode() {
        // For each direction
        for (int i = 0; i < nbQuadDir; ++i) // Delete the corresponding son
            if (sons[i])
                delete sons[i];
    }
...
template <typename T>
void display(QuadTree<T>* qt, std::string prefix = "", void(*prt)(T) = nullptr){ // Print the quadtree
    // If tree is not null
    if (qt != nullptr){
        // If tree is leaf
        if (qt->isLeaf()) {
